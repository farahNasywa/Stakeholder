...
const AddStakeholderPage = () => {
  ...
  // Pesan error field wajib ditampilkan dalam bahasa Inggris (permintaan
  // eksplisit), terlepas dari bahasa antarmuka (ID/EN) yang sedang aktif.
  // Hanya field bertanda bintang merah (*) di StakeholderFormFields.jsx
  // yang divalidasi di sini: Stakeholder Name, Stakeholder Type, Category,
  // Province, dan Regency/City. Field lain (Role, District, Contact, dsb.)
  // tetap opsional dan tidak divalidasi.
  const validateRequiredFields = () => {
    if (!form.name?.trim()) {
      return 'Stakeholder name is required.';
    }
    if (!form.stakeholderType) {
      return 'Please select a stakeholder type.';
    }
    if (!form.engagementCategory) {
      return 'Please select a stakeholder category.';
    }
    if (!form.location?.province) {
      return 'Please select a province.';
    }
    if (!form.location?.city) {
      return 'Please select a regency/city.';
    }
    return null;
  };
